...
export async function readApprovedSoftware(): Promise<string[]> {
  try {
    ...
    if (!fs.existsSync(EXCEL_PATH)) {
      console.warn(
        `❌ Approved software Excel file not found at: ${EXCEL_PATH}`,
      );
      console.warn(
        "Please ensure the file exists and OneDrive is synchronized.",
      );
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const areaNormalized = normalizeForComparison(areaStr).replace(
        /\s+/g,
        "_",
      );
      ...
      console.log(
        `  Fila ${i + 1}: area="${areaStr}" (norm: "${areaNormalized}"), puesto="${puestoStr}" (norm: "${puestoNormalized}"), software="${softwareTrimmed}"`,
      );
    ...
    console.log(
      `   - Software general: ${approvedSoftwareHierarchy.general.length}`,
    );
    console.log(`   - Áreas: ${approvedSoftwareHierarchy.areas.size}`);
    console.log(
      `   - Puestos específicos: ${approvedSoftwareHierarchy.puestos.size}`,
    );
    console.log(
      `   - Total único de software: ${approvedSoftwareCache.length}`,
    );
...
console.log(
  "📦 Módulo excel-utils cargado. Cache inicial vacío:",
  approvedSoftwareCache.length,
);
